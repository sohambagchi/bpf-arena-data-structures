...
## Core technical docs
- `docs/GUIDE.md` current architecture, build/test workflow, one-way mode, and extension points
...
## Source map
- `include/` data structure headers (`ds_msqueue`, `ds_vyukhov`, `ds_folly_spsc`, `ds_ck_*`, `ds_io_uring`, `ds_iouring_liburing`, `ds_kcov`, `ringbuf_bench`)
  - `include/ds_io_uring.h` BPF arena port of io_uring's SPSC ring memory model
  - `include/ds_iouring_liburing.h` io_uring/liburing faithful SPSC ring with four role-specific memory ordering patterns
  - `include/ds_kcov.h` BPF arena port of Linux kcov's flat append buffer
- `src/` relay / one-way / ringbuf apps (`skeleton_*.bpf.c` + `skeleton_*.c`)
  - `src/skeleton_io_uring.bpf.c` + `src/skeleton_io_uring.c` io_uring ring relay
  - `src/skeleton_iouring_liburing.bpf.c` + `src/skeleton_iouring_liburing.c` io_uring/liburing relay with role-specific ordering
  - `src/skeleton_kcov.bpf.c` + `src/skeleton_kcov.c` kcov buffer relay
- `usertest/` userspace-only pthread tests
- `scripts/usertests.py` maintained correctness test runner
- `scripts/benchmarking.py` maintained benchmark runner (`--one-way` for arena-vs-ringbuf comparisons)
