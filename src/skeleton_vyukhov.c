...
static struct skeleton_vyukhov_bpf *skel;
...
__attribute__((noinline)) void vyukhov_kernel_consume_trigger(void)
{
	asm volatile("" ::: "memory");
}
...
static int attach_programs(void)
{
	...
	struct bpf_uprobe_opts uprobe_opts = {
		...
		.func_name = "vyukhov_kernel_consume_trigger",
	};
	int err;
	...
	consume_link = bpf_program__attach_uprobe_opts(
		skel->progs.bpf_vyukhov_consume,
		getpid(),
		"/proc/self/exe",
		0,
		&uprobe_opts);
	...
	if (err)
		return err;
	skel->links.bpf_vyukhov_consume = consume_link;
...
{
	struct ds_vyukhov_head *head_ku = &skel->arena->global_ds_head_ku;
	struct ds_vyukhov_head *head_uk = &skel->arena->global_ds_head_uk;
	struct ds_kv data;
	...
	printf("UserThread: waiting for VyukhovKU initialization...\n");
	while (!stop_test) {
		if (head_ku->buffer)
			break;
	}
	...
	while (!stop_test) {
		if (!config.one_way && !uk_initialized) {
			if (!head_uk->buffer) {
				ret = ds_vyukhov_init_c(head_uk, VYUKHOV_QUEUE_CAPACITY);
				if (ret != DS_SUCCESS)
					continue;
			}
			uk_initialized = true;
		...
		DS_METRICS_RECORD_OP(&skel->arena->global_metrics, DS_METRICS_USER_CONSUMER, {
			ret = ds_vyukhov_pop_c(head_ku, &data);
		}, ret);
		if (ret == DS_SUCCESS) {
			int ins_ret;
			...
			DS_METRICS_RECORD_OP(&skel->arena->global_metrics, DS_METRICS_USER_PRODUCER, {
				ins_ret = ds_vyukhov_insert_c(head_uk, data.key, data.value);
			}, ins_ret);
...
static void trigger_kernel_consumer_on_exit(void)
{
	...
	if (uk_enqueued_count == 0) {
		vyukhov_kernel_consume_trigger();
		return;
	}
...
static int verify_data_structure(void)
{
	...
	printf("Verifying Vyukhov queues from userspace...\n");

	if (head_ku->buffer)
		ku_result = ds_vyukhov_verify_c(head_ku);
	if (head_uk->buffer)
		uk_result = ds_vyukhov_verify_c(head_uk);
...
static void print_statistics(void)
{
	...
	printf("\n============================================================\n");
	printf("                   VYUKHOV RELAY STATISTICS                 \n");
	printf("============================================================\n");
	...
	if (config.one_way)
		ds_metrics_print_oneway(&skel->arena->global_metrics, "Vyukhov MPMC");
	else
		ds_metrics_print(&skel->arena->global_metrics, "Vyukhov MPMC");
	printf("============================================================\n\n");
}
...
static void print_usage(const char *prog)
{
	...
	printf("Vyukhov relay test (kernel->user->kernel lanes)\n\n");
	printf("OPTIONS:\n");
	...
	printf("Flow:\n");
	printf("  inode_create -> VyukhovKU (kernel producer)\n");
...
int main(int argc, char **argv)
{
	...
	printf("Loading BPF program for Vyukhov relay...\n");
