...
__attribute__((noinline)) void ck_stack_upmc_kernel_consume_trigger(void)
{
	asm volatile("" ::: "memory");
}
...
static int attach_programs(void)
{
	...
	struct bpf_uprobe_opts uprobe_opts = {
		...
		.func_name = "ck_stack_upmc_kernel_consume_trigger",
	};
	int err;
	...
	consume_link = bpf_program__attach_uprobe_opts(
		skel->progs.bpf_ck_stack_upmc_consume,
		getpid(),
		"/proc/self/exe",
		0,
		&uprobe_opts);
	...
	if (err)
		return err;
	skel->links.bpf_ck_stack_upmc_consume = consume_link;
...
{
	struct ds_ck_stack_upmc_head *head_ku = &skel->arena->global_ds_head_ku;
	struct ds_ck_stack_upmc_head *head_uk = &skel->arena->global_ds_head_uk;
	struct ds_kv data;
	...
	printf("UserThread: waiting for CKStackUPMCKU initialization...\n");
	while (!stop_test) {
		if (skel->bss->initialized_ku)
			break;
	}
	if (stop_test)
		return NULL;
	...
	while (!stop_test) {
		if (!config.one_way && !uk_initialized) {
			if (!skel->bss->initialized_uk) {
				ds_ck_stack_upmc_init_c(head_uk);
				skel->bss->initialized_uk = true;
			}
			uk_initialized = true;
		}

		DS_METRICS_RECORD_OP(&skel->arena->global_metrics, DS_METRICS_USER_CONSUMER, {
			ret = ds_ck_stack_upmc_pop_c(head_ku, &data);
		}, ret);
		if (ret == DS_SUCCESS) {
			int ins_ret;
			...
			DS_METRICS_RECORD_OP(&skel->arena->global_metrics, DS_METRICS_USER_PRODUCER, {
				ins_ret = ds_ck_stack_upmc_insert_c(head_uk, data.key, data.value);
			}, ins_ret);
...
static int verify_data_structure(void)
{
	...
	printf("Verifying CK stack lanes from userspace...\n");

	ku_result = ds_ck_stack_upmc_verify_c(head_ku);
	uk_result = ds_ck_stack_upmc_verify_c(head_uk);
...
static void print_statistics(void)
{
	...
	printf("\n============================================================\n");
	printf("                CK STACK UPMC RELAY STATISTICS              \n");
	printf("============================================================\n");
	...
	printf("Stack states:\n");
	printf("  KU count=%llu\n", (unsigned long long)head_ku->count);
	printf("  UK count=%llu\n", (unsigned long long)head_uk->count);
	if (config.one_way)
		ds_metrics_print_oneway(&skel->arena->global_metrics, "CK Stack UPMC");
	else
		ds_metrics_print(&skel->arena->global_metrics, "CK Stack UPMC");
	printf("============================================================\n\n");
}
...
static void print_usage(const char *prog)
{
	...
	printf("CK Stack UPMC relay test (kernel->user->kernel lanes)\n\n");
	printf("OPTIONS:\n");
	...
	printf("Flow:\n");
	printf("  inode_create -> CKStackUPMCKU (kernel producer)\n");
...
int main(int argc, char **argv)
{
	...
	printf("Loading BPF program for CK stack UPMC relay...\n");
