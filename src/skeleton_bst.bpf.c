...
static __always_inline int handle_operation(struct ds_operation *op)
{
	...
	case DS_OP_INIT:
		...
		result = ds_bst_init_lkmm(ds_head);
		initialized = true;
		break;
		
	case DS_OP_INSERT:
		/* DS_API_INSERT: Call BST insert function */
		result = ds_bst_insert_lkmm(ds_head, op->kv.key, op->kv.value);
	...
	case DS_OP_DELETE:
		...
		result = ds_bst_delete_lkmm(ds_head, op->kv.key);
		if (result == DS_SUCCESS)
			bst_delete_count++;
		else
			bst_delete_retries++;
	...
	case DS_OP_SEARCH:
		...
		result = ds_bst_search_lkmm(ds_head, op->kv.key);
		bst_search_count++;
		break;
		
	case DS_OP_VERIFY:
		/* DS_API_INSERT: Call BST verify function */
		result = ds_bst_verify_lkmm(ds_head);
		break;
...
int BPF_PROG(lsm_inode_create, struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	ts = bpf_ktime_get_ns();
	result = ds_bst_insert_lkmm(ds_head, pid, ts);
