...
/* Skeleton Userspace Program for Concurrent Data Structure Testing
 * 
 * This program provides a framework for testing data structures where:
...
/**
 * workload_insert_only - Perform only insert operations
 */
static void workload_insert_only(struct ds_list_head *head, int thread_id, 
                                  struct thread_stats *stats)
{
	for (int i = 0; i < config.ops_per_thread && !stop_test; i++) {
		...
		int result = ds_list_insert(head, key, value);
 ...
 */
static void workload_search_only(struct ds_list_head *head, int thread_id,
                                  struct thread_stats *stats)
{
	for (int i = 0; i < config.ops_per_thread && !stop_test; i++) {
		__u64 key = generate_random_key(thread_id, i);
		...
		int result = ds_list_search(head, key, &value);
 ...
 */
static void workload_mixed(struct ds_list_head *head, int thread_id,
                            struct thread_stats *stats)
{
	for (int i = 0; i < config.ops_per_thread && !stop_test; i++) {
		__u64 key = generate_random_key(thread_id, i);
		...
		if (op_type < 5) {
			/* Insert */
			result = ds_list_insert(head, key, value);
			stats->inserts++;
		} else if (op_type < 8) {
			/* Search */
			result = ds_list_search(head, key, &value);
			stats->searches++;
		} else {
			/* Delete */
			result = ds_list_delete(head, key);
			stats->deletes++;
		}
