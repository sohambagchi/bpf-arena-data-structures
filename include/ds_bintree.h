 ...
 * 
 * This is a leaf-oriented BINTREE where:
...
enum ds_bintree_state {
	BINTREE_CLEAN = 0,  /* No operation in progress */
	BINTREE_DFLAG = 1,  /* Delete operation flagged */
	BINTREE_IFLAG = 2,  /* Insert operation flagged */
	BINTREE_MARK  = 3   /* Parent marked for deletion */
};

/* Node Types */
enum ds_bintree_node_type {
	BINTREE_NODE_INTERNAL = 0,
	BINTREE_NODE_LEAF     = 1,
	BINTREE_NODE_KEY_INF1 = 2,
	BINTREE_NODE_KEY_INF2 = 4,
	BINTREE_NODE_KEY_INFINITE = (BINTREE_NODE_KEY_INF1 | BINTREE_NODE_KEY_INF2)
};

/* Info record types */
enum ds_bintree_info_type {
	BINTREE_INFO_INSERT = 0,
	BINTREE_INFO_DELETE = 1
};
...
#define BINTREE_MAX_DEPTH 4

/* Sentinel key values (users must use keys < BINTREE_SENTINEL_KEY1) */
#define BINTREE_SENTINEL_KEY1 ((__u64)-2)  /* UINT64_MAX - 1 */
#define BINTREE_SENTINEL_KEY2 ((__u64)-1)  /* UINT64_MAX */

/* ========================================================================
 * DATA STRUCTURES
 * ======================================================================== */

struct ds_bintree_tree_node {
	enum ds_bintree_node_type type;
};

/**
 * struct ds_bintree_leaf - Leaf node containing actual data
 * @header: Common node header
 * @kv: Key-value pair stored in the leaf
 */
struct ds_bintree_leaf {
	struct ds_bintree_tree_node header;
    struct ds_kv kv;
};

/**
 * struct ds_bintree_internal - Internal routing node
 * @header: Common node header
 ...
 */
struct ds_bintree_internal {
	struct ds_bintree_tree_node header;
	struct ds_k key;
	struct ds_bintree_tree_node __arena *pLeft;
	struct ds_bintree_tree_node __arena *pRight;
	__u64 m_pUpdate;  /* Tagged pointer: (Info* & ~0x3) | State */
};

/**
 * struct ds_bintree_info - Base for Info records
 * @type: Info record type (BINTREE_INFO_INSERT or BINTREE_INFO_DELETE)
 */
struct ds_bintree_info {
	__u32 type;
};

/**
 * struct ds_bintree_iinfo - Insert operation context
 * @header: Common info header
 * @p: Parent internal node
 * @l: Leaf to replace
 * @new_internal: New internal node to insert
 */
struct ds_bintree_iinfo {
	struct ds_bintree_info header;
	struct ds_bintree_internal __arena *pParent;
	struct ds_bintree_internal __arena *pNew;
	struct ds_bintree_leaf __arena *pLeaf;
	bool bRightLeaf;
};

/**
 * struct ds_bintree_dinfo - Delete operation context
 * @header: Common info header
 ...
 */
struct ds_bintree_dinfo {
	struct ds_bintree_internal __arena *pGrandParent;
	struct ds_bintree_internal __arena *pParent;
	struct ds_bintree_leaf __arena *pLeaf;
...
struct ds_bintree_update_desc {
	union {
		struct ds_bintree_iinfo __arena *insert;
		struct ds_bintree_dinfo __arena *delete;
	};
	struct ds_bintree_update_desc __arena *next_retire;
};

struct ds_bintree_stats {
	__u32 total_inserts;
	__u32 total_deletes;
...
/**
 * struct ds_bintree_head - Tree head structure
 * @root: Root internal node
 * @count: Number of elements (excluding sentinels)
 */
struct ds_bintree_head {
	struct ds_bintree_stats stats;
	struct ds_bintree_internal __arena *root;
	struct ds_bintree_leaf __arena *leaf_inf1;
	struct ds_bintree_leaf __arena *leaf_inf2;
	__u64 count;
};

#define ds_bintree_node(ptr, type, member) arena_container_of(ptr, type, member)
 ...
 */
static inline __u64 bintree_make_update(void __arena *info, __u64 state)
{
 ...
 */
static inline __u8 bintree_get_bits(__u64 update)
{
 ...
 */
static inline __u64 bintree_get_ptr(__u64 update)
{
...
static inline bool bintree_is_internal(struct ds_bintree_tree_node __arena *node) {
	return node->type == BINTREE_NODE_INTERNAL;
}

static inline bool bintree_is_leaf(struct ds_bintree_tree_node __arena *node) {
...
/**
 * struct bintree_search_result - Context returned by search
 * @gp: Grandparent internal node
 * @p: Parent internal node
 * @l: Leaf node found
...
/**
 * bintree_search - Search for a key and return context
 * @head: Tree head
                               ...
                               struct bintree_search_result *res)
{
	...
	while (bintree_is_internal(pLeaf) && iterations < BINTREE_MAX_DEPTH && can_loop) {
		...
		cast_kern(pParent);
		updParent = (struct ds_bintree_update_desc __arena *) smp_load_acquire(&pParent->m_pUpdate);
...
/**
 * bintree_cas_child - CAS the appropriate child pointer of parent
...
/**
 * bintree_help_insert - Complete an insert operation
...
static inline void bintree_help_insert(struct ds_bintree_update_desc __arena *op)
{
	...
	struct ds_bintree_tree_node __arena *pLeaf= (struct ds_bintree_tree_node __arena *)op->insert->pLeaf;
...
/**
 * bintree_help_marked - Complete physical deletion
...
/**
 * bintree_help_delete - Attempt to mark parent and complete deletion
 * @op: Delete operation context
 * 
 * Steps:
...
static inline bool bintree_help_delete(struct ds_bintree_update_desc __arena *op)
{
	...
	/* Don't call bintree_help(res) to avoid deep recursion in BPF */
...
/**
 * bintree_help - Help complete a pending operation
 ...
 * 
 * Users must insert keys < BINTREE_SENTINEL_KEY1.
 * 
...
static inline int ds_bintree_init(struct ds_bintree_head __arena *head)
{
	...
	cast_user(leaf1);
	cast_user(leaf2);
	root->pLeft = (struct ds_bintree_tree_node __arena *)leaf1;
	root->pRight = (struct ds_bintree_tree_node __arena *)leaf2;
	root->m_pUpdate = bintree_make_update(NULL, BINTREE_CLEAN);
 ...
 * @head: Tree head
 * @key: Key to insert (must be < BINTREE_SENTINEL_KEY1)
                                     ...
                                     struct ds_kv kv)
{
	...
	while (iterations < BINTREE_MAX_DEPTH && can_loop) {
		...
		if (bintree_get_child(res.pParent, res.bRightLeaf, BINTREE_RELAXED) == (struct ds_bintree_tree_node __arena *)res.pLeaf) {
			...
			struct ds_bintree_iinfo __arena *iinfo = (struct ds_bintree_iinfo __arena *)bpf_arena_alloc(sizeof(*iinfo));
			if (!iinfo) {
				bpf_arena_free(pOp);
				bpf_arena_free(new_leaf);
				bpf_arena_free(new_internal);
...
static inline int ds_bintree_delete(struct ds_bintree_head __arena *head, struct ds_kv kv)
{
	...
	while (iterations < BINTREE_MAX_DEPTH && can_loop) {
		...
		res_cas = arena_atomic_cmpxchg(&res.pGrandParent->m_pUpdate, bintree_get_ptr(res.updGrandParent), new_up,
		                               ARENA_ACQ_REL, ARENA_RELAXED);
 ...
 * 
 * Checks:
 * - BINTREE property (left < parent <= right)
...
static inline const struct ds_metadata* ds_bintree_get_metadata(void)
{
	static const struct ds_metadata metadata = {
		...
		.node_size = sizeof(struct ds_bintree_leaf),
