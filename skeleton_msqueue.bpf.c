...
struct ds_msqueue_head __arena global_ds_head;
struct ds_msqueue_head __arena *ds_head;
...
static __always_inline int handle_operation(struct ds_operation *op)
{
	...
	case DS_OP_INIT:
		...
		result = ds_msqueue_init(ds_head);
		initialized = true;
		break;
		
	case DS_OP_INSERT:
		/* DS_API_INSERT: Call your insert function */
		result = ds_msqueue_insert(ds_head, op->key, op->value);
		break;
		
	case DS_OP_DELETE:
		/* DS_API_INSERT: Call your delete function */
		result = ds_msqueue_delete(ds_head, op->key);
		break;
		
	case DS_OP_SEARCH:
		/* DS_API_INSERT: Call your search function */
		result = ds_msqueue_search(ds_head, op->key, &op->value);
		break;
		
	case DS_OP_VERIFY:
		/* DS_API_INSERT: Call your verify function */
		result = ds_msqueue_verify(ds_head);
		break;
...
int trace_exec(struct trace_event_raw_sys_enter *ctx)
{
	...
	/* Perform insert operation (enqueue) */
 ...
 * operations (dequeue), testing concurrent deletions with userspace.
 */
...
int trace_exit(struct trace_event_raw_sys_enter *ctx)
{
	...
	/* Generate a key to delete (dequeue) */
	__u32 pid = bpf_get_current_pid_tgid() >> 32;
	key = (bpf_ktime_get_ns() ^ pid) % config_key_range;
	
	/* Perform delete operation (dequeue) */
...
int batch_operations(void *ctx)
{
	...
	for (i = 0; i < config_num_operations && i < 1000 && can_loop; i++) {
		...
		/* Alternate between inserts (enqueue) and searches */
		if (i % 2 == 0) {
			op.type = DS_OP_INSERT;
			op.key = i;
			op.value = i * 10;
